...
def incluirBD(df) :
    # CREATE TABLE ENVIGORPROVINCIAS(Provincia varchar(30), SubsectorTec varchar(30), NPublicadas varchar(30), Year varchar(30), lastupdate varchar(30));
    header = df.iloc[0]
    ...
    for i in range(1, df.shape[0]) :
        ...
        for j in range(2,len(row)) :
            ...
            querystring = """INSERT INTO exampledb.ENVIGORPROVINCIAS (Provincia, SubsectorTec, NPublicadas, Year, lastupdate) VALUES (""" + lista + """);"""
            #print(querystring)
...
def limpiarBD() :
    dbconnection = MySQLdb.connect(host='localhost',db='exampledb',
                              user='exampleuser', passwd='pimylifeup')
    querystringlimp = "DELETE FROM exampledb.ENVIGORPROVINCIAS"
    c = dbconnection.cursor()
...
df = dfs[0]
row = []
with open ('ENVIGORPROVINCIAS.csv', mode = 'a') as employee_file:
